# tradingagents/TradingAgents/tradingagents/agents/utils/investment_rag.py
import requests
...
def retrieve_investment_wisdom(query: str) -> str:
    ...
    try:
        resp = requests.post(
            "http://10.10.xx.xx:8000/search",  # 你的 RAG API 地址
            json={"query": query},
            timeout=10
        )
...
def retrieve_research_manager_wisdom_v0(query: str, context: dict = None, role: list = None) -> str:
    ...
    try:
        resp = requests.post(
            "http://10.10.xx.xx:9000/search",  # 你的 RAG API 地址

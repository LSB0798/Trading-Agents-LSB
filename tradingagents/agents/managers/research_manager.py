...
def create_research_manager_v0(llm, memory):
...
# =================================== v1 versions =================================== #
# [RAG] LLM generated query: 巴菲特在面临监管风险和市场波动时的投资策略，结合腾讯控股（0700.HK）的估值和护城河分析。
# [RAG] LLM generated query: 巴菲特关于市场波动与企业护城河的关系，结合腾讯的案例
...
"""
        ...
        for index in range(max_iterations):
            print('index =============================')
            """ 作用：每次调用 LLM 时，模型根据当前对话历史（messages）和系统提示（system_message），自主决定是否在响应中包含 tool_calls。
            LLM 根据系统提示和对话历史，自主判断是否需要检索外部智慧。如果需要，返回的 response 会包含 tool_calls 字段。
            
            优点:
                灵活性：模型可以根据不同股票、不同市场环境、不同辩论焦点，动态调整检索内容。
                针对性：模型能检索最相关的投资智慧，而不是每次都检索固定的泛化内容。
            潜在风险:
                查询质量不稳定：模型可能生成模糊或无关的查询，导致检索结果不理想。
                查询格式问题：如果模型生成的查询包含特殊字符或过长，可能影响 API 响应。"""
            try:
            ...
            if not response.tool_calls: # 检查 LLM 返回的响应中是否包含工具调用请求。如果为空（即模型选择不调用工具），则设置 final_response = response 并跳出循环；否则进入工具执行分支。
                ...
                break
            # 执行工具调用
            print('response.tool_calls : {}, type : {}'.format(response.tool_calls, type(response.tool_calls)))

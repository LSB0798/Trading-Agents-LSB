    ...
    def setup_graph(
        ...
        Args:
            selected_analysts (list): List of analyst types to include.
                - If use_master_analysts=False: values can be "market", "social", "news", "fundamentals"
                - If use_master_analysts=True: values can be "news" and master analyst names
            use_master_analysts (bool): If True, use parallel execution of master analysts.
        """
        ...
        if use_master_analysts:
            # 大师分析师模式：需要导入包装器
            from tradingagents.agents.hf_analysts_wrapper import get_master_analyst_node

            for analyst_name in selected_analysts:
                if analyst_name == "news":
                    # 保留原有新闻分析师（包含全球宏观新闻）
                    analyst_nodes["news"] = create_news_analyst(self.quick_thinking_llm)
                    delete_nodes["news"] = create_msg_delete()
                    tool_nodes["news"] = self.tool_nodes["news"]   # 保留工具节点（可能用不到）
                else:
                    # 大师分析师：不需要工具节点（数据通过适配层获取）
                    ...
                    tool_nodes[analyst_name] = None   # 无工具节点
        else:
            """
        ...
        if use_master_analysts:
            """
            # 并行执行所有的专家，然后汇聚
            # 大师模式：所有分析师并行执行，汇聚到 SignalAggregator
            workflow.add_node("SignalAggregator", signal_aggregator_node)

            # 所有分析师从 START 并行启动
            for analyst_name in analyst_nodes.keys():
                workflow.add_edge(START, f"{analyst_name.capitalize()} Analyst")
                ...
                if analyst_name == "news":
                    ...
                    workflow.add_conditional_edges(
                        f"{analyst_name.capitalize()} Analyst",
                        getattr(self.conditional_logic, f"should_continue_{analyst_name}"),
                        [current_tools, current_clear],
                    )
                    workflow.add_edge(current_tools, f"{analyst_name.capitalize()} Analyst")
                    workflow.add_edge(current_clear, "SignalAggregator")
                else:
                    # 大师分析师：直接到清理节点
                    workflow.add_edge(f"{analyst_name.capitalize()} Analyst", f"Msg Clear {analyst_name.capitalize()}")
                    workflow.add_edge(f"Msg Clear {analyst_name.capitalize()}", "SignalAggregator")

            # 汇聚节点连接到 Bull Researcher

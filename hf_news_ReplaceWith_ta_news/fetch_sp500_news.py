...
SP500_SYMBOLS = [
    ...
    "FFIV", "FDS", "FICO", "FAST", "FRT", "FDX", "FDXF", "FIS", "FITB", "FSLR",
...
def get_sp500_tickers():
    """直接返回预定义的标普500成分股列表"""
    logger.info(f"从预定义列表获取到 {len(SP500_SYMBOLS)} 只股票")
    return SP500_SYMBOLS

def fetch_news_for_ticker(ticker, days_back=14):
    """获取单只股票的新闻并保存"""
...
def main():
    ...
    logger.info(f"开始处理 {len(tickers)} 只股票")
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                if result:
                    success_count += 1
                    logger.info(f"✅ {ticker} 新闻保存成功")
                else:
                    logger.info(f"⚠️ {ticker} 无新闻或获取失败")
            except Exception as e:
                logger.error(f"❌ {ticker} 处理异常: {e}")
            time.sleep(0.1)  # 适当延迟避免触发限流

    logger.info(f"完成！成功获取 {success_count}/{len(tickers)} 只股票的新闻")
...
BNY	BNY Mellon	Financials	Asset Management & Custody Banks	New York City, New York	1995-03-31	0001390777	1784
...
SATS	EchoStar	Communication Services	Wireless Telecommunication Services	Englewood, Colorado	2026-03-23	0001415404	2008

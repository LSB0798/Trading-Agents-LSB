...
def get_model_v0(model_name: str, model_provider: ModelProvider, api_keys: dict = None) -> ChatOpenAI | ChatGroq | ChatOllama | GigaChat | None:
...
def get_model(model_name: str, model_provider: ModelProvider, api_keys: dict = None) -> ChatOpenAI | ChatGroq | ChatOllama | GigaChat | None:
    # ========== 侵入式修改：强制使用本地 Qwen3-32B ==========
    # 忽略传入的 model_name 和 model_provider，直接返回连接到本地 API 的 ChatOpenAI 实例
    LOCAL_API_BASE = "http://10.20.223.83:61273/v1"
    LOCAL_API_KEY = "dummy"
    LOCAL_MODEL_NAME = "qwen3_32B"
    
    # 直接返回配置好的 ChatOpenAI 客户端
    print(f"[get_model] 强制使用本地模型: {LOCAL_MODEL_NAME} @ {LOCAL_API_BASE}")
    return ChatOpenAI(
        model=LOCAL_MODEL_NAME,
        openai_api_key=LOCAL_API_KEY,
        openai_api_base=LOCAL_API_BASE,
        temperature=0.3,
        request_timeout=120,  # 本地模型可能响应较慢，增加超时
    )
